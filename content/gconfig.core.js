var guiconfig = {
	...
	resetToDefault : function() {
		...
		var dont_ask = { value : false };
		var reset_options = this.prompts.confirmCheck(null, "gui:config", this.getLocaleString("confirm-std"), this.getLocaleString("dont-ask"), dont_ask);
	...
	createPreferences : function() {
		var group, group_name,
			groupTab, groupPanel, groupBox,
			subgroup_name,
			subgroupBox, subgroupCaption,
			opt, optObject,
			newOption;
		var options_num = 0;
		var prefConfig = this.options;
	...
	updateOption : function(opt) {
		...
		optBox.parentNode.replaceChild(optObject, optBox);
		return true;
	},
	buildOption : function(opt, ignore_validation) {
		if (!this.validateOption(opt) && !ignore_validation) return false;
